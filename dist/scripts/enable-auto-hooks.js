...
try {
    ...
    if (!disable) {
        const proof = JSON.parse(await fs.readFile("docs/automatic-hook-probe.json", "utf8"));
        const version = (await run("codex", ["--version"])).stdout.trim();
        if (proof.codex !== version ||
            !["mcp", "code_bridge"].every((mode) => proof.results.some((r) => r.mode === mode &&
                r.exitCode === 0 &&
                r.calls === 2 &&
                !r.sawRaw &&
                r.sawKept &&
                r.sawFeedback)))
            throw new SetupError("Run the automatic hook host probe successfully before installation.");
    }
    let original = "";
    try {
    ...
    const isOurs = (h) => h.type === "mcp_tool" &&
    ...
    const retained = existing.map((g) => {
        ...
        if (g.hooks.some((h) => !isOurs(h)))
            throw new SetupError("AlphaOptimizer shares a hook group with other handlers; no hook configuration was changed.");
        if (found)
            throw new SetupError("Duplicate automatic hook definitions; no hook configuration was changed.");
        found = true;
    ...
    lines.on("line", (line) => {
        try {
            ...
            if (message.error)
                task.reject(new SetupError("Codex configuration API rejected a request."));
            else
    ...
    if (!disable) {
        const listing = listings.parse(await rpc("hooks/list", { cwds: [process.cwd()] }));
        if (listing.data.some((entry) => entry.errors.length))
            throw new SetupError("Codex reported hook configuration errors; auto mode was not enabled.");
        const ours = listing.data
            .flatMap((entry) => entry.hooks)
            .find((h) => h.sourcePath === hooksPath &&
            h.server === "alphaoptimizer" &&
            h.tool === "process_tool_result");
        if (!ours)
            throw new SetupError("Codex did not discover the new hook; auto mode was not enabled.");
        ...
        const verified = listings.parse(await rpc("hooks/list", { cwds: [process.cwd()] }));
        if (!verified.data
            .flatMap((entry) => entry.hooks)
            .some((h) => h.key === ours.key &&
            h.currentHash === ours.currentHash &&
            h.trustStatus === "trusted"))
            throw new SetupError("Exact hook trust was not confirmed; auto mode was not enabled.");
    }
    await rpc("config/value/write", {
        filePath: configPath,
    ...
    if (latestInstructions !== instructions) {
        ...
        throw new SetupError("Instructions changed concurrently; rerun the installer.");
    }
