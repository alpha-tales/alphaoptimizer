...
for (const entry of [...manifest.files, "package.json", "package-lock.json", "LICENSE"] as string[]) {
...
if (!license.ok) throw new Error("Cannot bundle Node without its license notices");
...
try {
  ...
  const result = JSON.parse((status.content as Array<{ text: string }>)[0].text);
  if (result.state !== "awaiting_hook_verification") throw new Error("Bundled MCP did not initialize with key-only configuration");
} finally {
  await client.close();
