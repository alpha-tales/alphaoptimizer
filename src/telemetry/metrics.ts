...
type Options = {
  ...
  flushMs?: number;
  maxFileBytes?: number;
  // Injection point for testing slow/unavailable disks, never a request-path await.
  writeBatch?: (batch: string) => Promise<void>;
};

export class BufferedMetrics implements MetricSink {
  ...
  readonly health = { accepted: 0, written: 0, dropped: 0, writeFailures: 0 };
  ...
  private schedule(): void {
    if (this.timer || this.pending || this.closed || !this.queue.length) return;
  ...
  private async drain(): Promise<void> {
    ...
    while (remaining > 0) {
      ...
      let bytes = 0;
      while (
        batch.length < Math.min(this.batchSize, remaining) &&
        this.queue.length
      ) {
        const lineBytes = Buffer.byteLength(this.queue[0]);
        if (bytes + lineBytes > this.maxFileBytes) break;
      ...
      try {
        ...
        if (this.health.writeFailures === 1)
          process.stderr.write(
            "alphaoptimizer: metrics write failed; some metrics were dropped\n",
          );
      }
    }
  }
