...
try {
    ...
    await Promise.all(modes.map(async (mode) => {
            ...
            try {
                ...
                if (calls === 1) {
                    ...
                    if (mode === "code" || mode === "code_bridge") {
                        ...
                        const name = mode === "mcp"
                            ? (() => { const tool = specs.find(t => t.namespace?.includes("fixture") || t.name.includes("fixture")); if (!tool)
                                throw new Error("Fixture MCP tool is not exposed by this host"); return tool.name; })()
        ...
        if (process.env.ALPHA_PROBE_PLUGIN_ROOT) {
            ...
            for (let i = flags.length - 1; i >= 0; i--) {
                if (flags[i].startsWith("mcp_servers.alphaoptimizer.") || flags[i].startsWith("hooks.PostToolUse="))
                    flags.splice(i, 1);
            }
        }
        let stdout = "", stderr = "";
    ...
    await fs.writeFile(path.join(root, process.env.ALPHA_PROBE_PLUGIN_ROOT ? "docs/plugin-hook-probe.json" : "docs/automatic-hook-probe.json"), JSON.stringify(report, null, 2) + "\n");
