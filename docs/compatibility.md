...
server. It installs an invocation-only PostToolUse hook, performs a harmless printf, and inspects the
...
`select_evidence` MCP tool to receive optimized evidence. No hook is installed automatically.
...
The legacy `handle_hook` remains advisory; the opt-in installer uses the new processor and a
separate code-mode emission policy. It does not enable replacement for paths missing status or
