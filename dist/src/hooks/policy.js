...
- Do not recursively optimize AlphaOptimizer calls. Do not attempt to compress images, structured JSON, streaming or known sensitive output; preserve those results. Jev is used only when ALPHAOPTIMIZER_JEV_API_KEY is configured.
...
${POLICY_END}`;
export function mergeOutputPolicy(original, enabled) {
    const start = original.indexOf(POLICY_START);
    const end = original.indexOf(POLICY_END);
    if (start >= 0 !== end >= 0 || (start >= 0 && end < start))
        throw new Error("Incomplete AlphaOptimizer policy markers; existing instructions were not changed.");
    if (start >= 0)
