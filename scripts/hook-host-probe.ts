...
const hookCommand = (hook: string) => `${process.platform === "win32" ? "& " : ""}${quote(process.execPath)} ${quote(hook)}`;
try {
  ...
  const report = {
    ...
    scope:
      "Real CLI hooks with synthetic local model transport; not desktop or production model validation",
    results,
  };
  await fs.writeFile(
    path.resolve("docs/hook-host-probe.json"),
    JSON.stringify(report, null, 2) + "\n",
  );
  console.log(JSON.stringify(report, null, 2));
  const probes = results as Array<{
    exitCode: number;
    hookReceipt: unknown;
    modelRequestSawOriginal: boolean;
    modelRequestSawReplacement: boolean;
  }>;
  if (
    probes.some((probe) => probe.exitCode !== 0 || !probe.hookReceipt) ||
