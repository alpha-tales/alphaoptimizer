...
try {
  ...
  try {
    ...
    const health = JSON.parse((status.content as Array<{text: string}>)[0].text);
  ...
  try {
    ...
    assert.equal(JSON.parse((result.content as Array<{text: string}>)[0].text).autoMode, "filter");
  } finally { await fresh.close(); }
  console.log(
    "PASS: packed installation and fresh plugin cache boot from unrelated cwd; hooks ship, filtering defaults on, missing key is reported, native dependencies initialize automatically.",
  );
} finally {
