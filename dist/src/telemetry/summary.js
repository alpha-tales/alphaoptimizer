...
import { METRIC_FILE, MetricSchema } from "./metrics.js";
export async function readMetrics(directory) {
    let names;
    try {
        names = (await fs.readdir(directory)).filter((name) => METRIC_FILE.test(name));
    }
    catch (error) {
        if (error.code === "ENOENT")
            return { events: [], skipped: 0, dropCounters: 0 };
        throw error;
    }
    const candidates = [];
    let skipped = 0;
        ...
        try {
            ...
            for (const line of content.split("\n")) {
                ...
                try {
                    ...
                    drops.set(stream, Math.max(drops.get(stream) ?? 0, event.loggerDropped ?? 0));
                }
                catch {
                    skipped++;
                }
...
export function summarizeMetrics(events) {
    ...
    return {
        measurement: "Retained local metrics only. Payload estimates use ceil(UTF-16 characters / 4); these are NOT measured Codex token or cost savings.",
        since: events[0]?.at ?? null,
        until: events.at(-1)?.at ?? null,
        selectionRequests: selections.length,
        automaticHooks: {
            inspected: hooks.length,
            replacementRequests: hooks.filter((e) => e.decision === "replacement_requested").length,
            ...
            requestedPayloadReductionTokensEstimate: hooks
                ...
                (e.outputTokensEstimate ?? 0), 0),
            reasons: Object.fromEntries([...new Set(hooks.map((e) => e.reason))].map((reason) => [
                reason,
                hooks.filter((e) => e.reason === reason).length,
            ])),
            deliveryNote: "Hook decisions, not host delivery receipts. Host timeouts or other hooks can leave the original visible.",
        ...
        eventCountsNote: "Selection and tool records describe different stages of a request; do not add them as independent requests.",
        groups: [...groups.entries()].map(([group, entries]) => {
            const successful = entries.filter((e) => e.status === "ok" && e.outputTokensEstimate !== null);
            const input = successful.reduce((n, e) => n + e.inputTokensEstimate, 0);
            const output = successful.reduce((n, e) => n + e.outputTokensEstimate, 0);
            return {
                group,
                ...
                payloadReductionPercentEstimate: input
                    ...
                    : null,
                fallbacks: entries.filter((e) => e.fallback !== "none" || e.provider === "fallback").length,
                ...latency(entries.map((e) => e.durationMs)),
            };
        }),
        provider: {
            attemptedRequests: selections.filter((e) => e.providerAttempted).length,
            successfulClassifications: selections.filter((e) => e.provider === "success").length,
            requestsWithoutUsage: selections.filter((e) => e.providerAttempted && e.providerInputTokens === null).length,
            knownInputTokens: selections.reduce((n, e) => n + (e.providerInputTokens ?? 0), 0),
            knownOutputTokens: selections.reduce((n, e) => n + (e.providerOutputTokens ?? 0), 0),
            ...latency(selections
        ...
        toolResponses: [
            ...
            return {
                ...
                responseTokensEstimate: records.reduce((n, e) => n + (e.responseTokensEstimate ?? 0), 0),
