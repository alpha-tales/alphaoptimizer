...
export class EvidenceStore {
  ...
  constructor(
    ...
    private readonly policy = {
      ...
      quotaPolicy: "oldest_unprotected" as "oldest_unprotected" | "reject",
    },
  ) {
    if (
      !Number.isSafeInteger(policy.maxStoreBytes) ||
    ...
    const usedPages = this.db.pragma("page_count", { simple: true }) as number;
    if (usedPages > pages) {
      this.db.close();
      throw new Error(
        "Existing database exceeds configured disk quota; increase maxStoreBytes",
      );
    }
      ...
      this.db
        .transaction(() => {
          const bytes = fs
            ...
            .reduce(
              (total, name) =>
                total + fs.statSync(path.join(this.artifactDir, name)).size,
              0,
            );
          if (bytes > this.rawBudget)
            throw new Error(
              "Existing raw files exceed configured disk quota; increase maxStoreBytes",
            );
        })
        .immediate();
    } catch (error) {
  ...
  private recoverArtifactFiles(): void {
    ...
    for (const name of fs.readdirSync(this.artifactDir)) {
      ...
      else if (!exists || match[2]) fs.rmSync(file, { force: true });
    }
  }

  cleanupExpired(now = new Date().toISOString()): number {
    try {
      return this.db
        .transaction(() => {
          ...
          const expired = this.db
            .prepare(
              "select artifact_id from artifacts a where expires_at is not null and expires_at <= ? and not exists (select 1 from artifact_leases l where l.artifact_id = a.artifact_id and l.expires_at > ?)",
            )
            .all(now, Date.now()) as Array<{ artifact_id: string }>;
      ...
      result = this.db
        .transaction(() => {
          ...
          if (!this.getArtifact(id)) {
            ...
            const rows = this.db
              .prepare(
                `select a.artifact_id, a.byte_length,
            ...
            for (const row of this.db
              ...
              .all() as Array<{ artifact_id: string; chunk_id: string }>) {
              if (refs.has(row.chunk_id)) refs.add(row.artifact_id);
            }
            let used = rows.reduce((total, row) => total + row.byte_length, 0);
            const evict = rows.filter(
              (row) => !row.pinned && !refs.has(row.artifact_id),
            );
            if (
              used + incoming > Math.floor(this.rawBudget / 4) &&
              ...
              throw new Error(
                "Store quota reached; protected evidence retained",
              );
            }
            for (const row of evict) {
              if (used + incoming <= Math.floor(this.rawBudget / 4)) break;
              const original = this.artifactPath(row.artifact_id);
            ...
            const diskBytes = fs
              ...
              .reduce(
                (sum, name) =>
                  sum + fs.statSync(path.join(this.artifactDir, name)).size,
                0,
              );
            if (diskBytes + incoming > this.rawBudget) {
              throw new Error(
                "Insufficient transactional disk headroom; increase maxStoreBytes or remove expired evidence",
              );
            }
          }
          ...
          this.replaceChunks(
            artifact.artifactId,
            chunkText(artifact.artifactId, observation.content ?? ""),
          );
          if (options.leaseId)
            this.db
              .prepare(
                "insert into artifact_leases values (?, ?, ?) on conflict(lease_id) do update set expires_at = excluded.expires_at",
              )
    ...
    return this.db
      .transaction(() => {
        const updated = this.db
          .prepare(
            "update artifact_leases set expires_at = ? where lease_id = ? and artifact_id = ?",
          )
  ...
  private captureObservationLocked(
    ...
    const existingEvent = this.db
      ...
      .get(eventId) as { artifact_id: string } | undefined;
    ...
    const tx = this.db.transaction(() => {
      ...
      `,
        ...
        .run(
          ...
          artifact.captureCompleteness,
          artifact.privacyClass,
          artifact.truncationFlag ? 1 : 0,
          artifact.createdAt,
          artifact.expiresAt,
        );
      this.db
  ...
  replaceChunks(artifactId: string, chunks: Chunk[]): void {
    const tx = this.db.transaction(() => {
      ...
      const insertFts = this.db.prepare(
        "insert into chunks_fts(text, chunk_id, artifact_id) values (?, ?, ?)",
      );
  ...
  getArtifact(
    ...
    const row = scope
      ? (this.db
          .prepare(
            "select * from artifacts where artifact_id = ? and workspace_id = ? and session_id = ?",
          )
          .get(artifactId, scope.workspaceId, scope.sessionId) as any)
      : (this.db
  ...
  readArtifactRange(
    ...
    const content = Buffer.alloc(
      Math.max(0, Math.min(stat.size - boundedStart, maxBytes + 4)),
    );
    try {
    ...
    if (content.length > 0 && (content[0] & 0b1100_0000) === 0b1000_0000) {
      throw new Error(
        `Invalid UTF-8 cursor for artifact ${artifactId}: ${boundedStart}`,
      );
    }
    const safeLength = utf8SafePrefixLength(
      content,
      requestedEnd - boundedStart,
    );
    ...
    const verifiedContent =
      startByte === 0 && maxBytes >= stat.size
  ...
  searchArtifactText(
    ...
    chunks: Array<Chunk & { partial: boolean; sourceChunkIds: string[] }>;
    ...
    const chunks: Array<
      Chunk & { partial: boolean; sourceChunkIds: string[] }
    > = [];
    const matchedRows = new Set<Chunk>();
    let used = 0,
      coveredEnd = charOffset,
    ...
    while ((match = pattern.exec(content))) {
      ...
      if (hitBytes > remaining || chunks.length >= 50) {
        if (!chunks.length)
          throw new Error(
            "Search byte budget is smaller than the matching text",
          );
      ...
      let start = Math.max(
        charOffset,
        match.index - Math.floor((available - hitBytes) / 8),
      );
      if (start > 0 && /[\uDC00-\uDFFF]/.test(content[start])) start++;
      const prefix = content.slice(start, match.index);
      const tail = boundedUtf8Prefix(
        content.slice(scannedChar),
        available - Buffer.byteLength(prefix) - hitBytes,
      );
      ...
      const sources = rows.filter(
        (row) => row.endByte > startByte && row.startByte < endByte,
      );
      chunks.push({
        artifactId,
        chunkId: shortId(
    ...
    const omittedChunks = [...matchedRows].filter(
      (row) =>
        !chunks.some(
          (window) =>
            window.startByte <= row.startByte && window.endByte >= row.endByte,
        ),
    ).length;
    const nextCursor =
      nextOffset === null
  ...
  upsertCriterion(input: {
    ...
    const criterion: Criterion = {
      ...
      observedRevision:
        input.observedRevision ?? existing?.observed_revision ?? null,
      unresolvedQuestion:
        input.unresolvedQuestion ?? existing?.unresolved_question ?? null,
    ...
    `,
      ...
      .run(
        ...
        criterion.workspaceId,
        criterion.sessionId,
        criterion.text,
        criterion.provenance,
        criterion.status,
        JSON.stringify(criterion.evidenceRefs),
        criterion.observedRevision,
        criterion.unresolvedQuestion,
        criterion.createdAt,
        criterion.updatedAt,
      );
    return criterion;
  ...
  listCriteria(workspaceId: string, sessionId: string): Criterion[] {
    ...
    `,
      ...
      .all(workspaceId, sessionId) as any[];
...
function criterionFromRow(row: any): Criterion {
  return {
...
function utf8SafePrefixLength(buffer: Buffer, requestedLength: number): number {
