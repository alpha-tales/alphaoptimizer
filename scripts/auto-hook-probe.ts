...
const text =
  Array.from({ length: 120 }, (_, i) =>
    i === 60
      ? omitted
      : `Routine processing item ${i}: background processing completed normally.`,
...
try {
  ...
  await fs.writeFile(
    fixture,
    `import {McpServer} from ${JSON.stringify(pathToFileURL(path.join(sdk, "server/mcp.js")).href)};import {StdioServerTransport} from ${JSON.stringify(pathToFileURL(path.join(sdk, "server/stdio.js")).href)};const s=new McpServer({name:"fixture",version:"1"});s.registerTool("emit",{inputSchema:{},annotations:{readOnlyHint:true,destructiveHint:false,openWorldHint:false}},async()=>({content:[{type:"text",text:${JSON.stringify(text)}}]}));await s.connect(new StdioServerTransport());`,
  );
  await Promise.all(
      ...
      const api = http.createServer(async (req, res) => {
        ...
        try {
          ...
          if (calls > 1) {
            ...
            const outputs = inputs.filter((item: { type?: string }) =>
              /tool_call_output|function_call_output/.test(item.type ?? ""),
            );
          ...
          if (calls === 1) {
            ...
            const specs: Spec[] = (request.tools ?? []).flatMap((t: Spec) =>
              t.tools ? t.tools.map((s) => ({ ...s, namespace: t.name })) : [t],
            );
            toolNames = specs
              .filter(
                (t) =>
                  ["exec", "exec_command", "shell_command"].includes(t.name) ||
            ...
            if (mode === "code" || mode === "code_bridge") {
              ...
              if (!exec) throw new Error("Code-mode exec tool is not exposed");
              const hookInput = {
          ...
          scenarioError = (error as Error).message;
          res.writeHead(500);
          res.end();
        }
      });
