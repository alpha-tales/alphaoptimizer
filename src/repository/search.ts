...
export async function searchRepository(input: {
  ...
  const allowed = await Promise.all(
    (input.allowlist ?? []).map((entry) => fs.realpath(path.resolve(entry))),
  );
  ...
  async function rg(extra: string[]): Promise<string> {
    signal.throwIfAborted();
    try {
  ...
  async function readCandidate(
    file: string,
  ): Promise<{ file: string; bytes: Buffer } | null> {
    signal.throwIfAborted();
    ...
    try {
      ...
      if ((await fs.realpath(absolute)) !== absolute) return null;
      ...
      if (
        ++scannedFiles > limits.maxScanFiles ||
        (scannedBytes += stat.size) > limits.maxScanBytes
      ) {
        throw new Error(
          "Repository scan limit exceeded; narrow the query or workspace",
        );
      }
      ...
      if (
        ["ENOENT", "ELOOP", "EACCES", "ENOTDIR"].includes(
  ...
  function excerpt(
    snapshot: { file: string; bytes: Buffer },
    line: number,
    reason: string,
  ): RepositoryExcerpt {
  ...
  if (exact.trim()) {
    const results: RepositoryExcerpt[] = [];
    for (const line of exact.split(/\r?\n/)) {
      ...
      if (snapshot) {
        const snapshotLines =
          snapshot.bytes.toString("utf8").match(/[^\n]*(?:\n|$)/g) ?? [];
        if (
          snapshotLines[Number(event.data.line_number) - 1] !==
  ...
  const results: Array<RepositoryExcerpt & { score: number }> = [];
