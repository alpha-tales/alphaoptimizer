...
function serverScope(workspace: string) {
...
server.registerTool(
  "read_artifact",
  {
    title: "Read Artifact",
    description:
      "Retrieve a bounded artifact range, or case-insensitive literal match windows. Pass query and the returned cursor to continue search.",
    inputSchema: {
      workspace: z.string(),
      sessionId: z.string().optional(),
      artifactId: z.string(),
      ...
      async () => {
        ...
        if (input.query) {
          return textResult({
            ...
            search: engine.store.searchArtifactText(
              input.artifactId,
              input.query,
              input.maxBytes,
              scope,
        ...
        return textResult({
          artifact: engine.store.getArtifact(input.artifactId, scope),
          range: engine.store.readArtifactRange(
            input.artifactId,
            input.startByte,
            input.maxBytes,
            scope,
...
server.registerTool(
  "record_criterion",
  {
    ...
    description:
      "Record explicit or inferred task criteria without claiming they are verified.",
    inputSchema: {
      workspace: z.string(),
      sessionId: z.string().optional(),
      text: z.string(),
...
server.registerTool(
  ...
  async (input) => {
    const scope = serverScope(input.workspace);
    const criteria = engine.store.listCriteria(
      scope.workspaceId,
      scope.sessionId,
    );
    return textResult({
      ...
      gaps:
        criteria.length === 0
          ...
          : criteria.filter(
              (criterion) =>
                criterion.status === "unknown" || criterion.status === "failed",
            ),
    });
  },
);
...
server.registerTool(
  "handle_hook",
  {
    title: "Handle Hook",
    description:
      "Normalize a supported Codex hook event and return an advisory host-specific response.",
    inputSchema: HookEventSchema.shape,
  },
  async (input) =>
    textResult(
      await handleHook(engine, { ...input, sessionId: config.sessionId }),
    ),
);
...
server.registerTool(
  "process_tool_result",
  {
    ...
    description:
      "Internal PostToolUse hook. Called automatically by the host; returns pass-through or bounded tool feedback. Never approves or executes tools.",
    inputSchema: HostToolEventSchema.shape,
  },
  async (input, extra) => {
    hookEventsReceived++;
    const feedback = await processToolResult(engine, input, extra.signal);
    if (feedback.continue === false) replacementRequests++;
