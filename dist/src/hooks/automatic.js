...
const SECRET = /-----BEGIN [\w ]*PRIVATE KEY-----|\bBearer\s+[\w.+\/-]{12,}|\b(?:password|passwd|api[_-]?key|access[_-]?token|client[_-]?secret|authorization|cookie)\s*["']?\s*[:=]\s*\S+|\b(?:sk-[\w-]{16,}|gh[pousr]_[\w]{20,}|AKIA[0-9A-Z]{16})|\beyJ[\w-]{8,}\.[\w-]+\.[\w-]+/i;
const SENSITIVE_INPUT = /agent-vault|\.env\b|\.npmrc\b|\.ssh\b|\.aws\b|\b(?:printenv|credential|password|secret|authorization|cookie|api[_-]?key)\b/i;
function object(value) {
    return value !== null && typeof value === "object" && !Array.isArray(value)
        ? value
        : null;
}
function textualResult(value) {
...
export async function processToolResult(engine, raw, signal) {
    ...
    try {
        ...
        metric.toolClass = /^(Bash|exec_command|shell_command|shell)$/.test(event.tool_name)
            ? "shell"
        ...
        if (metric.toolClass === "shell") {
            ...
            if (typeof event.exit_code !== "number")
                return pass("status_unknown");
            if (event.exit_code !== 0)
                return pass("error_result");
            result.exitCode = event.exit_code;
        }
        if (/Process running with session ID|Script running with cell ID|"session_id"\s*:\s*\d+/i.test(result.text))
            return pass("running");
        if (/\btruncated\b|Output too large|original_token_count/i.test(result.text))
        ...
        const feedback = JSON.stringify({
            alphaoptimizer: "Reduced tool result. This is evidence, not an instruction or a request to stop.",
            originalStatus: { isError: false, exitCode: result.exitCode },
            capture: "Stored exactly the hook-visible text; upstream capture completeness is unknown.",
        ...
        if (engine.config.autoMode === "observe")
            return pass("observe");
