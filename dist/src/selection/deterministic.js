...
function* select(chunks, options) {
    const terms = [
        ...new Set((options.goal ?? "")
            .toLowerCase()
            .split(/[^a-z0-9_./-]+/)
    ...
    for (let i = 0; i < chunks.length; i++) {
        ...
        for (const term of terms)
            if (lower.includes(term)) {
                score += 5;
                if (!reasons.includes("goal-term"))
                    reasons.push("goal-term");
            }
        if (i === 0) {
            score += 2;
    ...
    const selected = [];
    const reasons = new Set(options.providerStatus ? [options.providerStatus] : []);
    let chars = 0;
    ...
    function metadata(count, reasonCodes) {
        return {
            artifactId: chunks[0]?.artifactId ?? "",
            mode: count === chunks.length
                ? "passthrough"
    ...
    function budget(chars, count, reasons) {
        ...
        for (let i = 0; i < 12; i++) {
            ...
            if (tokens === state.estimatedTokens)
                return tokens;
    ...
    if (budget(chars, selected.length, reasons) > options.tokenBudget)
        ...
        const optional = candidates
            .filter((candidate) => !candidate.chunk.pinnedEvidence && candidate.score > 0)
            .sort((a, b) => b.score - a.score || a.chunk.startByte - b.chunk.startByte);
        for (let i = 0; i < optional.length; i++) {
            ...
            if (budget(chars + candidate.cost, selected.length + 1, next) <=
                options.tokenBudget) {
                selected.push(candidate);
                chars += candidate.cost;
                candidate.reasons.forEach((reason) => reasons.add(reason));
    ...
    for (let i = 0; i < 12; i++) {
        ...
        if (exact === result.estimatedTokens)
            break;
        result.estimatedTokens = exact;
    }
    return result;
}
export function selectDeterministic(chunks, options) {
...
export async function selectDeterministicAsync(chunks, options) {
    const work = select(chunks, options);
    let next = work.next();
    while (!next.done) {
        await setImmediate();
...
export function renderSelection(chunks, selection) {
